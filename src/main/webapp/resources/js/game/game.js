...
function show_player_tile() {
    ...
    for (let i = 0; i < playerTile.length; i++) {
        ...
        div.id = id;
        div.innerHTML += '<img onclick="player_tile_click(' + id + ')" id= "img' + id + '" src="/resources/images/game/' + path + '.svg" class="tile player-tile no-drag">';
...
function player_tile_refresh() {
    ...
    playerBoard.innerHTML = '<div class="set-player-board" onclick="set_player_board_click()"><img src="/rummi/resources/images/game/set.svg" class="tile-set" id="tile-set"></div>'; // 추가 버튼 먼저 띄우기
...
function mainBoardDivAddTile(i, div, divId, tile) {
    ...
    div.innerHTML += '<img onclick="main_board_tile_click(' + id + ')" id="' + id + '" src="/resources/images/game/' + path + '.svg" class="main-board-tile board-tile no-drag">';
}
...
function completeBundeFun(i, arr) {
    ...
    if (isComplete) {
        ...
        for (let j = 0; j < count; j++) {
            ...
            //console.log(arr);
            bundle_div.innerHTML += '<img onclick="main_board_tile_click(' + id + ')" id="' + id + '" src="/rummi/resources/images/game/' + path + '.svg" class="main-board-tile board-tile no-drag">';
        }
        ...
        if(tile777 || tile789){
            ...
            for(let j = k; j < count; j++){
                ...
                const path = id.substring(1);
                //console.log(arr);
                div.innerHTML = '<img onclick="main_board_tile_click(' + id + ')" id="' + id + '" src="/rummi/resources/images/game/' + path + '.svg" class="main-board-tile board-tile no-drag">';
                mainBoard.appendChild(div);
            }
        }
...
function refresh_click() {
    ...
    if (mainBoardBundle.length != 0) {
        // mainBoard.innerHTML = '<div class="set-main-board" onclick="set_board_click(clickTile)"><img src="images/set.svg" class="tile-set" id="tile-set1"></div>';
        mainBoard.innerHTML = '';
...
function set_player_board_click() {
    ...
    for (let i = 0; i < clickTile.length; i++) {
        ...
        if (location == 'mainBoard') {
            ...
            let div = document.createElement("div");
            div.id = id;
            div.innerHTML += '<img onclick="player_tile_click(' + id + ')" id= "img' + id + '" src="rummi/resources/images/game/' + path + '.svg" class="tile no-drag">';
            playerBoard.insertBefore(div, set_tile);
        } else {
            tile_click_shadow('img' + id);
        }
